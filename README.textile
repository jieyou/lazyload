...
`$(selector).offset().top/left` are always zero(http://bugs.jquery.com/ticket/3037) when `display:none` is set. As we can't get real offset of the element, the picture is always loaded when we set both `display:none` and 'skip_invisible = false'. It go against the intention of lazyload, so we decide to remove this attribute from options. lazyload will ignore pictures with display attribute set to none. Lazyload will not be triggered when display attribute is changed(i.e. remove_invisible.html). It may cause some trouble sometimes but scrolling your mouse will definately help!
